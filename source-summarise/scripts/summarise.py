#!/usr/bin/env python3
# Part of the nopilot-co-utilities Claude Code plugin (utilities:source-summarise).
# STUB — scaffolding only. Invoked by skills/source-summarise/SKILL.md via
...
STUB. The mechanical I/O is implemented (reads the manifest; writes a
...
A future version may call an LLM API directly.
...
def apply_summary(path, s):
    ...
    if s.get("core_arguments"):
        lines += ["**Core arguments:**", ""] + [f"- {a}" for a in s["core_arguments"]] + [""]
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Summarise sources in an enriched batch (STUB).")
    ap.add_argument("--batch", required=True)
    ap.add_argument("--manifest", default="sources.json")
    ap.add_argument("--only", action="append", default=[])
    ap.add_argument("--limit", type=int, default=0)
    ap.add_argument("--summary-json", help="model-produced summaries (schema in the docstring/SKILL)")
    ...
    def match(e):
        return (not sel or str(e.get("n")) in sel or e.get("id") in sel
                or os.path.splitext(e["file"])[0] in sel or e["file"] in sel)

    if not a.summary_json:
        print("STUB: source-summarise is scaffolding only.")
        print("Provide model-produced summaries via --summary-json. Schema (keys = id|file|n):")
        print('  {"<id|file|n>": {"summary","precis","position","core_arguments":[],"comment_reaction"}}')
    ...
    for e in manifest["sources"]:
        ...
        if not match(e):
            continue
        key = next((k for k in (e.get("id"), e.get("file"), str(e.get("n"))) if k in smap), None)
        if not key:
            continue
