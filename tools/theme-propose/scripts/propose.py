#!/usr/bin/env python3
# Part of the nopilot-co-studios tool-bench (tools/theme-propose).
# Invoked by skills/theme-propose/SKILL.md via
...
def render_proposal_md(prop):
    lines = [
        ...
        "",
        "## Editorial approach (overall)",
        "",
        prop.get("editorial") or "_TODO_",
        "",
        "## Categorisation guidance",
        "",
        prop.get("categorisation_guidance") or "_TODO_",
        "",
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Propose a theme framework + freeze a theme manifest, non-destructive."
    )
    ap.add_argument("--batch", required=True)
    ap.add_argument(
        "--manifest-name", default="sources.json", help="batch manifest filename"
    )
    ap.add_argument(
        "--proposal-json",
        help="model-produced proposal to materialise (schema in docstring)",
    )
    ap.add_argument(
        "--adopt",
        help="an agreed proposal/manifest JSON to freeze into theme-manifest.json",
    )
    ap.add_argument(
        "--manifest",
        default="theme-manifest.json",
        help="output manifest filename within --batch",
    )
    ...
    if a.adopt:
        ...
        with open(out_path, "w", encoding="utf-8") as fh:
            ...
            fh.write("\n")
        print(
            f"adopted -> {out_path}  ({len(out['themes'])} themes; governs theme-cluster/theme-entity)"
        )
        return 0
    ...
    if a.proposal_json:
        ...
        print(
            "review/edit theme-proposal.json, then: theme-propose --batch <dir> --adopt theme-proposal.json"
        )
        return 0
    ...
    with open(ctx_path, "w", encoding="utf-8") as fh:
        json.dump(
            {"count": len(digest), "sources": digest}, fh, ensure_ascii=False, indent=2
        )
        fh.write("\n")
    print(
        f"theme-context.json written: {len(digest)} summarised source(s) -> {ctx_path}"
    )
    print(
        "Non-destructive scan. Next: the model proposes a framework from this digest, then"
    )
    print(
        "  theme-propose --batch <dir> --proposal-json proposal.json   (materialise for review)"
    )
    print(SCHEMA_HINT)
    return 0
