#!/usr/bin/env python3
# Part of the nopilot-co-studios tool-bench (tools/youtube-transcript).
...
def extract_video_id(s: str) -> str:
    ...
    if re.fullmatch(r"[0-9A-Za-z_-]{11}", s):
        return s
    m = re.search(
        r"(?:v=|/v/|youtu\.be/|/embed/|/shorts/|/clip/)([0-9A-Za-z_-]{11})", s
    )
    if m:
        return m.group(1)
...
def fetch_oembed(video_id: str):
    ...
    try:
        ...
        return requests.get(
            url, timeout=10, headers={"User-Agent": "yt-transcript"}
        ).json()
    except Exception:
        pass
    try:
...
def native_chapters(info: dict):
    ...
    for ch in (info or {}).get("chapters") or []:
        if ch.get("start_time") is None:
            continue
        out.append(
            {"start": float(ch["start_time"]), "title": (ch.get("title") or "").strip()}
        )
...
def whisper_fallback(video_id: str, model_size: str, keep_audio: bool):
    ...
    print(
        f"Transcribing with faster-whisper ({model_size}, cpu/int8) ...",
        file=sys.stderr,
    )
    ...
    print(
        ...
        f"(p={info.language_probability:.2f}, {info.duration:.0f}s audio)",
        file=sys.stderr,
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--lang", default="en", help="comma-separated language priority, e.g. 'de,en'"
    )
    ap.add_argument(
        "--paragraph",
        action="store_true",
    ...
    ap.add_argument(
        "--session-title", help="front-matter session_title (else the video title)"
    )
    ap.add_argument(
        "--speaker", help="front-matter speaker (cannot be derived; pass it in)"
    )
    ap.add_argument(
        "--event", help="front-matter event (cannot be derived; pass it in)"
    )
    args = ap.parse_args()
    ...
    print(
        f"OK: wrote {args.out} ({words} words, video {video_id}, via {source}{suffix})"
    )
    return 0
