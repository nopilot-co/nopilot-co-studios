#!/usr/bin/env python3
# Part of the nopilot-co-studios tool-bench (tools/theme-entity).
# Invoked by skills/theme-entity/SKILL.md via
...
The mechanical assembly is implemented: for each theme in themes.json it
...
def render_theme(theme, spec, out_dir, batch_dir):
    ...
    out = [
        "## Summary",
        "",
        s.get("summary") or "_TODO: thematic summary (model-supplied via --spec)_",
        "",
    ]
    if s.get("precis"):
    ...
    out += (
        [f"- {x}" for x in s["notable"]]
        if s.get("notable")
        else ["_TODO: notable contributions_"]
    ) + [""]
    out += ["## Key disagreements", ""]
    out += (
        [f"- {x}" for x in s["disagreements"]]
        if s.get("disagreements")
        else ["_TODO: highlight key disagreement_"]
    ) + [""]
    out += [
        "## Comment-reaction assessment",
        "",
        s.get("comment_assessment")
        or "_TODO: assess the reaction across comment sections_",
        "",
    ]
    ...
    for m in timeline:
        ...
        out.append(
            f"- **{when}** — {_link(m, out_dir, batch_dir)} · {m.get('author') or 'Unknown'}"
        )
    out.append("")
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Build theme entity docs.")
    ap.add_argument("--batch", required=True)
    ap.add_argument(
        "--themes",
        default="themes.json",
        help="themes.json within --batch (from theme-cluster)",
    )
    ap.add_argument(
        "--spec", help="model-produced synthesis per theme (schema in docstring/SKILL)"
    )
    ...
    if not os.path.isfile(tpath):
        sys.stderr.write(
            f"error: themes file not found: {tpath} (run theme-cluster first)\n"
        )
        return 2
    ...
    for th in themes:
        ...
        if not a.quiet:
            print(
                f"  ✓ {os.path.relpath(p, a.batch)}  ({th.get('source_count', len(th.get('members', [])))} sources)"
            )

    print(f"\nbuilt {len(written)} theme entity doc(s) -> {out_dir}/")
    if not a.spec:
        print(
            "  note: semantic sections are TODO placeholders; pass --spec (model-produced) to fill them."
        )
    return 0
