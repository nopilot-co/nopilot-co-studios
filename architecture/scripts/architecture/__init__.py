"""architecture-studio orchestrator.

Deterministic glue for the architecture studio's spec + ADRs + diagram
chain. The LLM judgment — identifying systems, specifying flows, designing
integrations, writing ADRs — lives in the plugin's skill. This package
handles only mechanics: scaffolding the per-engagement store, schema
validation, referential-integrity invariants, ADR CRUD, and the design-
studio chain.
...
def architecture_root() -> Path:
    """Shared studios-level architecture store."""
    docket = docket_root()
    if docket:
        return docket / "architecture"

    env = os.environ.get("STUDIOS_PROJECT_ROOT")
    if env:
        return Path(env).expanduser() / "architecture"
    ...
    return Path.home() / "context" / "studios" / "architecture"


PLUGIN_ROOT = Path(__file__).resolve().parent.parent.parent  # .../architecture/
